...
export class MapComponent  implements OnInit{
  departements: any[] = [  { nom: 'Ain', code: '01' },
  { nom: 'Aisne', code: '02' },
  { nom: 'Allier', code: '03' },
...
affiche_choix:boolean=false
bouton_choix:boolean=false
...
selectedDepartment: string = ''; 
  ...
  gps()
  {
    ...
    this.bouton_choix=true;
    this.affiche_choix=false;
    this.chargementMAP();
  ...
  Dselection(){
    this.bouton_choix=true;
    this.affiche_choix=true;
   
  }
  chargementMAPChoice() {
    const postData = {
      departement_code: this.selectedDepartment // Utilisation de la valeur sélectionnée
    };
  
    this.http.post<any>('http://127.0.0.1:5000/generate_map', postData).subscribe(
      (data: any) => {
        document.open();
        document.write(data.map_html);
        document.close();
